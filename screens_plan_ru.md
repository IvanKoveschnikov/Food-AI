...
Цель: показать календарь и список блюд за выбранный день.

Основные блоки:
- AppBar с заголовком `AppStrings.home` («Главная»).
...
- Список блюд за выбранную дату:
  ...
  - каждая карточка блюда:
    - фото (если есть, через `storage_service.dart`);
    - название блюда;
    - краткое описание;
    - дата/время (опционально);
    - тап по карточке открывает экран «Подробнее о блюде» (`/dish/:id`).
- FAB / кнопки добавления:
  - «Добавить блюдо» (`AppStrings.addDish`):
    - открывает вариант выбора (с фото или без фото);
  - «Добавить без фото» (`AppStrings.addDishNoPhoto`):
    - ведёт на `/add-no-photo` с выбранной датой.

Состояния:
- Загрузка (крутилка) пока подтягиваются блюда.
- Пустое состояние:
...
Цель: добавить блюдо руками, при желании всё равно использовать ИИ по тексту.

Элементы:
- AppBar: заголовок `AppStrings.addDishNoPhoto`.
- Поля:
  - «Название блюда»;
  - текстовое поле для описания;
  - опционально — выбор даты (по умолчанию выбранная на главном экране).
- Кнопка «Анализировать описание»:
  - отправляет текст в `analyzeDishDescription` (`ai_service.dart`);
  - подставляет название блюда, описание и список продуктов.
- Список продуктов / добавление своих:
  - логика аналогична экрану «Фото».
- Кнопка «Сохранить»:
  - аналогично `_save()` в `camera_screen.dart`, но без загрузки фото;
  - создаёт запись в `saved_dishes` и связи в `dish_products`.
