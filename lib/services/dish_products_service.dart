...
Future<void> setDishProducts(String dishId, List<String> productIds, {List<String> userAddedIds = const []}) async {
  await supabase.from('dish_products').delete().eq('dish_id', dishId);
  for (final pid in productIds) {
    await supabase.from('dish_products').insert({
      'dish_id': dishId,
      'product_id': pid,
...
Future<void> addDishProduct(String dishId, String productId, {bool isUserAdded = false}) async {
...
Future<void> removeDishProduct(String dishId, String productId) async {
  await supabase.from('dish_products').delete().eq('dish_id', dishId).eq('product_id', productId);
}
